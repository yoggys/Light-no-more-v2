...
public class CharlatanScene extends Scene {
	...
	// konstruktor
	public CharlatanScene(SceneManager gsm) {
		this.gsm = gsm;
	...
	public void draw(Graphics2D g) {
		...
		if ((int) Player.champions.get(0).payHeal() == 0 && (int) Player.champions.get(1).payHeal() == 0
				&& (int) Player.champions.get(2).payHeal() == 0) {
			currentChoice[row] = 0;
			row = 0;
		}
		...
		for (int i = 0; i < Player.champions.size(); i++) {
			...
			g.drawString(String.valueOf((int) Player.champions.get(i).payHeal()),
					427 + (200 * i) + (int) (check[i] * 12), 615);
		}
		if (state == 1) {
			g.setColor(Color.WHITE);
